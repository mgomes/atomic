...
func TestCLIUsesAtomicName(t *testing.T) {
	t.Parallel()

	var output bytes.Buffer
	if got, want := command.New("test", &output, &output).Name, "atomic"; got != want {
...
func TestCLIBackupAndRestoreWorkflow(t *testing.T) {
	...
	if output, err := run(t, "--config", configPath, "init"); err != nil {
		t.Fatalf("atomic init returned error: %v\n%s", err, output)
	}
	...
	if !strings.Contains(string(initialized), "configuration_id:") {
		t.Error("atomic init config has no persistent configuration_id")
	}
	source := filepath.Join(root, "source")
	if err := os.MkdirAll(source, 0o700); err != nil {
		t.Fatalf("MkdirAll(source) returned error: %v", err)
	}
	if err := os.WriteFile(filepath.Join(source, "hello.txt"), []byte("hello from Atomic"), 0o600); err != nil {
...
`
  ...
  "plans": {
	...
	if err != nil {
		t.Fatalf("atomic status returned error: %v\n%s", err, output)
	}
	for _, text := range []string{
		...
		if !strings.Contains(output, text) {
			t.Errorf("atomic status output = %q, want %q", output, text)
		}
	}
	if strings.Contains(output, "\x1b[") {
		t.Errorf("atomic status redirected output contains ANSI escapes: %q", output)
	}
}
...
func run(t *testing.T, args ...string) (string, error) {
	...
	err := app.Run(context.Background(), append([]string{"atomic"}, args...))
