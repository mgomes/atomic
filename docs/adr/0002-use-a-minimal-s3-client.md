...
Atomic will use a small, standard-library S3 client that implements AWS
...
Content-Range; Atomic never falls back to downloading the complete object.
...
Atomic will not implement Signature Version 2, SigV4 streaming payloads,
...
Atomic repositories contain immutable encrypted objects: blocks of about 4
...
Atomic now owns sensitive authentication code and must maintain golden SigV4
...
- The AWS SDK handles more endpoint and credential variants, but its API and
  dependency surface substantially exceed Atomic's five fixed operations.
