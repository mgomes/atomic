...
can run `./atomic gc` after a hard crash to remove unreachable encrypted
blocks, manifests, and temporary objects.

`atomic verify` is the slower integrity path: it authenticates every encrypted
...
periodic `atomic run --full PLAN` rereads and hashes every source file as well,
covering filesystems whose change metadata is coarse or unavailable.

Running `atomic` with no command opens the dashboard. The TUI also has an
...
`--config` has highest precedence, followed by `ATOMIC_CONFIG`, followed by the
...
`atomic init` writes a random `configuration_id` that namespaces plan history
and retention inside a repository. Keep that value when moving the config. A
hand-written config may omit it; Atomic then derives a stable ID from the
...
folder: `~/.local/share/atomic/repository` on Linux,
`~/Library/Application Support/atomic/repository` on macOS, and
`%LocalAppData%\atomic\repository` on Windows. This keeps large backup data out
...
When `atomic init` is given an explicit `--config` path, it instead creates a
...
Atomic keeps at least 14 snapshots and every snapshot newer than 90 days. A
...
`atomic daemon` runs the scheduler in the foreground. This is useful with a
...
Atomic executable at the path from which it was installed.
...
Linux init systems can still supervise `atomic daemon` directly but are not
...
systems Atomic requires that it have no group or other permissions; on Windows
...
`0600` file. macOS extended ACLs are rejected. On Windows, Atomic enforces the
...
- Version one preserves regular files, directories, their `rwx` permission
  ...
  permissions with its read-only attribute. Atomic also preserves portable
